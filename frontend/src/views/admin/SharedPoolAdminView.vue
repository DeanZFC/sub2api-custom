...
const revenueColumns = [{ key: 'user', label: '发布用户' }, { key: 'amounts', label: '收益汇总' }, { key: 'wallet', label: '钱包状态' }, { key: 'last_request_at', label: '最近请求' }]
...
async function loadRevenue() {
  ...
  try { const result = await listSharedRevenue({ search: search.value || undefined, page: revenuePagination.value.page, page_size: revenuePagination.value.pageSize }); revenue.value = result.items || []; revenuePagination.value.total = result.total || 0 } finally { loadingRevenue.value = false }
}
async function loadRecords() {
  loadingRecords.value = true
  try { const result = await listSharedRevenueRecords({ search: search.value || undefined, page: recordPagination.value.page, page_size: recordPagination.value.pageSize }); records.value = result.items || []; recordPagination.value.total = result.total || 0 } finally { loadingRecords.value = false }
}
