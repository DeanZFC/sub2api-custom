...
const openPlan = (plan?: TestPlan) => {
  if (plan) {
    ...
    editingPlan.value = { name: '', test_definition_id: activePlanTypeId.value && activePlanTypeId.value > 0 ? activePlanTypeId.value : (orderedTypes.value.find(type => type.enabled)?.id || 0), group_id: null, account_id: null, target_mode: 'group', model_id: '', reasoning_effort: null, cron_expression: '*/30 * * * *', enabled: true, max_results: 50 }
  }
