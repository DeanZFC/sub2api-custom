  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <div v-else-if="mode === 'mine'" class="card overflow-x-auto">
        <div v-if="selectedCards.length" class="flex items-center gap-3 rounded-t-lg bg-primary-50 px-5 py-3 text-sm text-primary-700 dark:bg-primary-900/20 dark:text-primary-300">批量编辑账号 <button class="font-medium underline" @click="selectedCards = []">清除选择</button><span>已选择 {{ selectedCards.length }} 个</span></div>
        <table class="w-full min-w-[760px] text-left text-sm">
          <thead class="border-b border-gray-200 text-xs uppercase text-gray-500 dark:border-dark-700"><tr><th class="w-12 px-5 py-3"><input type="checkbox" class="rounded border-gray-300 text-primary-600" :checked="selectedCards.length === cards.length && cards.length > 0" @change="selectedCards = ($event.target as HTMLInputElement).checked ? cards.map(card => card.id) : []" /></th><th class="px-5 py-3">名称</th><th class="px-5 py-3">最近请求</th><th class="px-5 py-3">状态</th><th class="px-5 py-3">调度</th><th class="px-5 py-3">分组</th><th class="px-5 py-3">用量</th><th class="px-5 py-3 text-right">操作</th></tr></thead>
          <tbody class="divide-y divide-gray-100 dark:divide-dark-700"><tr v-for="card in cards" :key="card.id" class="hover:bg-gray-50 dark:hover:bg-dark-800"><td class="px-5 py-4"><input type="checkbox" class="rounded border-gray-300 text-primary-600" :checked="selectedCards.includes(card.id)" @change="selectedCards = ($event.target as HTMLInputElement).checked ? [...selectedCards, card.id] : selectedCards.filter(id => id !== card.id)" /></td><td class="px-5 py-4"><div class="flex flex-col"><span class="font-medium text-gray-900 dark:text-white">{{ card.display_name }}</span><span class="text-xs text-gray-500">{{ card.platform }} · 并发 {{ card.concurrency_limit }} · 倍率 {{ card.sell_rate }}x</span></div></td><td class="px-5 py-4 text-sm text-gray-500">{{ timeText(card.last_called_at) }}</td><td class="px-5 py-4"><span class="rounded-full px-2.5 py-1 text-xs font-medium" :class="card.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'">{{ labels[card.status] || card.status }}</span></td><td class="px-5 py-4"><button role="switch" :aria-checked="card.status === 'active'" class="relative inline-flex h-5 w-9 rounded-full" :class="card.status === 'active' ? 'bg-primary-500' : 'bg-gray-200 dark:bg-dark-600'" @click="toggle(card)"><span class="absolute top-0.5 h-4 w-4 rounded-full bg-white transition-transform" :class="card.status === 'active' ? 'translate-x-4' : 'translate-x-0.5'" /></button></td><td class="px-5 py-4 text-sm text-gray-500">共享账号</td><td class="px-5 py-4 text-sm text-gray-500">{{ card.total_call_count.toLocaleString() }} 次</td><td class="px-5 py-4"><div class="flex justify-end gap-2"><button class="btn btn-secondary btn-sm" @click="openTest(card)">测试</button><button class="btn btn-secondary btn-sm" @click="openEdit(card)">编辑</button><button class="btn btn-secondary btn-sm text-red-500" @click="remove(card)">删除</button></div></td></tr></tbody>
        </table>
      </div>
