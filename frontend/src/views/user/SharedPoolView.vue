    ...
    <div class="space-y-6">
      ...
      <div v-else-if="mode === 'mine'" class="card overflow-x-auto">
        <table class="w-full min-w-[760px] text-left text-sm">
          <thead class="border-b border-gray-200 text-xs uppercase text-gray-500 dark:border-dark-700"><tr><th class="px-5 py-3">账号</th><th class="px-5 py-3">平台</th><th class="px-5 py-3">状态</th><th class="px-5 py-3">并发</th><th class="px-5 py-3">倍率</th><th class="px-5 py-3">调用次数</th><th class="px-5 py-3 text-right">操作</th></tr></thead>
          <tbody class="divide-y divide-gray-100 dark:divide-dark-700"><tr v-for="card in cards" :key="card.id" class="hover:bg-gray-50 dark:hover:bg-dark-800"><td class="px-5 py-4 font-medium text-gray-900 dark:text-white">{{ card.display_name }}</td><td class="px-5 py-4 text-gray-500">{{ card.platform }}</td><td class="px-5 py-4"><span class="rounded-full px-2.5 py-1 text-xs font-medium" :class="card.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'">{{ labels[card.status] || card.status }}</span></td><td class="px-5 py-4 text-gray-500">{{ card.concurrency_limit }}</td><td class="px-5 py-4 text-gray-500">{{ card.sell_rate }}x</td><td class="px-5 py-4 font-semibold text-primary-600">{{ card.total_call_count.toLocaleString() }}</td><td class="px-5 py-4"><div class="flex justify-end gap-2"><button class="btn btn-secondary btn-sm" @click="openTest(card)">测试连接</button><button role="switch" :aria-checked="card.status === 'active'" class="relative h-5 w-9 rounded-full" :class="card.status === 'active' ? 'bg-primary-500' : 'bg-gray-200 dark:bg-dark-600'" @click="toggle(card)"><span class="absolute top-0.5 h-4 w-4 rounded-full bg-white transition-transform" :class="card.status === 'active' ? 'translate-x-4' : 'translate-x-0.5'" /></button><button class="btn btn-secondary btn-sm" @click="openEdit(card)">编辑</button><button class="btn btn-secondary btn-sm text-red-500" @click="remove(card)">删除</button></div></td></tr></tbody>
        </table>
      </div>
