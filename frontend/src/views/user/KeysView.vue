                ...
                >
                  ...
                  <GroupBadge
                    ...
                    :user-rate-multiplier="
                      userGroupRates[row.fallback_group.id]
                    "
                    ...
                    :peak-rate-multiplier="
                      row.fallback_group.peak_rate_multiplier
                    "
                  />
                  <span
        ...
        <div>
          ...
          >
            <template #selected="{ option }">
              <GroupBadge
                v-if="
                  option && (option as unknown as GroupOption).value !== null
                "
                :name="(option as unknown as GroupOption).label"
                :platform="(option as unknown as GroupOption).platform"
                :subscription-type="
...
const selectedGroupIdForSelector = computed(() => {
  ...
  return groupSelectorTarget.value === "fallback"
    ? key.fallback_group_id
    : key.group_id;
});

const groupButtonRefKey = (keyId: number, target: GroupSelectorTarget) =>
  `${keyId}:${target}`;
...
const fallbackGroupOptions = computed(() => {
  const primary = groups.value.find(
    (group) => group.id === formData.value.group_id,
  );
  return [
...
const groupSelectorOptions = computed(() => {
  ...
  return groupOptions.value.filter(
    (option) =>
      option.value !== key.group_id && option.platform === primaryPlatform,
  );
});
...
const openGroupSelector = (key: ApiKey, target: GroupSelectorTarget) => {
  ...
  if (
    ...
    const buttonEl = groupButtonRefs.value.get(
      groupButtonRefKey(key.id, target),
    );
    if (buttonEl) {
...
const changeGroup = async (key: ApiKey, newGroupId: number | null) => {
  ...
  try {
    ...
    appStore.showSuccess(
      t(
        fallbackCleared
          ? "keys.groupChangedFallbackClearedSuccess"
          : "keys.groupChangedSuccess",
      ),
    );
    loadApiKeys();
  } catch (error) {
    appStore.showError(
      t(
        target === "fallback"
          ? "keys.failedToChangeFallbackGroup"
...
const handleSubmit = async () => {
  ...
  if (formData.value.fallback_group_id !== null) {
    ...
    const fallback = groups.value.find(
      (group) => group.id === formData.value.fallback_group_id,
    );
    if (
      !primary ||
      !fallback ||
