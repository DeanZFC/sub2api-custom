export default {
    accounts: {
      ...
      openai: {
        ...
        codexTicketAccountEnabled: '账号级 292 打票',
        codexTicketAccountEnabledDesc: '仅对当前 OpenAI OAuth / Setup Token 账号生效。启用后由账号独立打票，不再依赖网关全局开关。',
        codexTicketFailClosed: '缺票时禁用模型',
        codexTicketFailClosedDesc: '默认开启。没有有效 292 门票时暂停对应模型；关闭后仍会打票，但缺票时继续放行请求。',
        codexTicketHarvestProxies: '专用打票代理',
        codexTicketHarvestProxiesDesc: '可选择多个代理用于打票轮换，不会改变账号业务请求使用的代理。未配置可用代理时不会打票。',
        codexTicketLegacyProxyFallback: '当前账号仍使用旧网关打票代理；只有选择或清空代理后才会切换为账号级代理配置。',
