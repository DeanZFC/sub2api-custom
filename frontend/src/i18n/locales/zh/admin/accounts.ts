export default {
    accounts: {
      ...
      openai: {
        prism: {
          title: 'Prism 实验通道',
          description: '默认关闭，支持纯文本 Responses 和 Chat Completions。关闭后恢复原有通道。',
          contextHint: '每次请求都会创建独立的 Prism 项目，客户端须发送完整对话上下文。模型沿用请求模型及账号已有的模型映射。',
          unsupportedHint: '不支持客户端工具、previous_response_id 和图像。',
          usageHint: '上游未返回用量时，不收取 Token 费用。',
          authMode: '认证方式',
          authModeAccount: '使用账号认证（推荐）',
          authModeCookie: '手动 Cookie',
          authModeAccountHint: '使用本账号已保存的 OpenAI 认证自动建立 Prism 会话。是否接受该认证由 Prism 决定；若被拒绝，可切换为手动 Cookie 并填写登录 Prism 后的会话 Cookie。',
          cookie: 'Prism Cookie',
          cookiePlaceholder: '填写您的 Prism 会话 Cookie',
          cookieKeepPlaceholder: '留空保留已保存的 Cookie',
          cookieHint: '手动 Cookie 模式首次启用时必填。保存后不回显 Cookie。',
          cookieConfiguredHint: '已保存 Cookie。留空表示保留，填写新值即可替换。',
          cookieRequired: '使用手动 Cookie 模式前请填写 Prism Cookie。',
          timeout: '总超时（秒）',
          timeoutHint: '30–600 秒，包含项目创建和回复生成；默认 180 秒。',
          timeoutInvalid: 'Prism 总超时须为 30–600 秒的整数。',
          advanced: '高级设置',
          actionId: 'Conversation Action ID',
          actionIdPlaceholder: '留空使用内置版本',
          actionIdHint: '仅当 Prism 会话操作变更时覆盖，填写抓包取得的 42 位十六进制 ID。',
          actionIdInvalid: 'Prism Conversation Action ID 须为 42 位十六进制字符。'
        },
        ...
        codexFingerprintModeDesc: '按账号稳定并隔离 Codex 身份标识。它只改写身份、缓存和会话标识，不等同于代理 IP 或网络出口。',
        ...
        antiDegrade: '账号保护',
        antiDegradeDesc: '可立即为现有账号启用保护。通用策略仅约束并发，身份策略还固定设备和传输；不能保证上游模型质量。',
        antiDegradeEnabled: '已启用',
        antiDegradeActiveMode: '当前策略',
        antiDegradeMode: '保护策略',
        antiDegradeModeHint: '启用前可选择策略；启用后选择新策略，核对配置后可立即应用。',
        antiDegradeApplying: '正在应用账号保护…',
        antiDegradeActiveHint: '上方“当前策略”为服务端已保存配置。关闭会还原此策略管理的设置，通用策略不改变身份、代理和传输。',
        antiDegradeApplyStrategy: '立即应用所选策略',
        antiDegradeConfiguredTransport: '目标传输配置',
        antiDegradeConfiguredIdentity: '目标身份配置',
        antiDegradeRuntimeHint: '这里展示的是目标配置；全局 TLS 开关、代理、插件传输和连接复用会影响实际出站连接。',
        antiDegradeDisable: '关闭并还原',
        antiDegradeDisableTitle: '关闭账号防降智？',
        antiDegradeDisableMessage: '关闭会立即还原此保护策略管理的设置；通用并发保护不会改动身份、代理和传输。',
        antiDegradeDisableConfirm: '确认关闭',
        antiDegradeApplied: '账号保护策略已应用',
        antiDegradeReverted: '账号防降智已关闭，原设置已恢复',
        antiDegradeFailed: '更新账号防降智失败',
        protectionPolicy: '账号保护策略',
        protectionPolicyDesc: '沿用现有 Codex 身份和 TLS 设置，只根据上游最终结果调整运行时并发。',
        protectionEnabled: '启用保护策略',
        protectionAdaptive: '自适应并发',
        protectionAdaptiveDesc: '连续出现上游 429/5xx 后降低运行时并发，账号保存的并发仍是最高上限。',
        protectionAutomatic: '自动调整',
        protectionObserve: '仅观察',
        protectionMinConcurrency: '运行时最低并发',
      ...
      poolModeInfo:
        '启用后，上游错误不会标记本地账号限流或错误，适用于上游指向另一个 sub2api 实例的场景。',
      ...
      proxy: '代理',
      proxyPool: '代理出口（可多选）',
      proxyPoolHint: '选择一个代理时沿用原有逻辑；选择多个代理时均匀分配请求，每个代理均使用下方设置的账号并发数。',
