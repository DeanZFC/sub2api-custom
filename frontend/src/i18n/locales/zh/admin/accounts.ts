export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '新建账号默认选择单机多窗口；存量未配置账号保持关闭。保留官方设备、会话和完整收敛模式；单机多窗口额外提供稳定窗口身份与 Mac TLS 配置，保留独立对话缓存，不调整并发或隔离账号。',
        ...
        codexTurnTicketDesc: '显示已配置模型的可用状态，292 的剩余时间同时考虑门票和路由 Cookie 有效期。仅在启用缺票拦截时，缺票或 292 缺少有效 Cookie 才会暂停对应模型。',
        codexTicketAccountEnabled: 'Codex 292 / 332 打票',
        codexTicketAccountEnabledDesc: '打票使用账号主代理，未配置代理时直连；业务多代理池可能使用其他出口。Pro 使用 292，Team 使用 332；门票默认最多有效 240 秒，提前 60 秒续票。292 所需路由 Cookie 自动采集并在同账号模型间共享，不跨账号使用。',
        codexTicketFailClosed: '缺票时禁用模型',
        codexTicketFailClosedDesc: '默认开启。没有有效门票或 292 缺少有效路由 Cookie 时暂停对应模型；关闭后仍会打票，但缺票时继续放行请求。',
        ...
        codexTicketDiagnostics: {
          ...
          cookieReady: '账号路由 Cookie：剩余 {seconds} 秒',
          cookieMissing: '账号路由 Cookie：缺失或已过期',
          ...
          errors: {
            ...
            cookie_missing: '路由 Cookie 缺失或已过期',
            no_proxy: '账号代理缺失或无效，请检查账号代理配置',
            proxy_cooldown: '账号出口正在冷却或并发已满，等待重试',
            canceled: '打票请求已取消',
