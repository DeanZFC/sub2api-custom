export default {
    ...
    channels: {
      ...
      form: {
        ...
        unnamed: '未命名',
        splitDefaultPrices: '按默认价拆分',
        splitDefaultPricesSuccess: '已按各模型默认价拆分，请检查价格并点击更新保存',
        splitDefaultPricesUnchanged: '未发现默认价格不同的模型；已有价格保持不变',
        pricingChangedDuringSync: '定价在同步期间已修改，请重新同步',
    ...
    riskControl: {
      ...
      noGroups: '暂无可用分组',
      userWhitelist: '用户白名单',
      userWhitelistHint: '所选用户跳过内容审计，不产生审计记录、通知或自动封禁。对所有分组和模型生效；留空则不豁免任何用户。',
      userWhitelistEmpty: '尚未添加白名单用户',
      modelFilter: '模型范围',
