export default {
    tests: {
      protection: {
        ...
        title: '自动质量保护',
        description: '检测异常时暂停账号调度；后续检测通过且所有异常解除后恢复。手动关闭调度的账号不会被检测或自动恢复。',
        accountOnly: '请选择某个账号或分组内所有账号后启用；分组检测不修改账号调度。',
        pauseOnFailure: '检测执行失败时暂停账号',
        minSamples: '最少请求样本数',
        metric: '判断指标', operator: '比较方式', threshold: '阈值',
        above: '大于', below: '小于', addThreshold: '添加异常阈值', removeThreshold: '删除阈值',
        vote: '由用户投票判断结果',
        rejectAbove: '不通过票数大于此值时暂停',
        passAtLeast: '通过票数达到此值时可恢复',
        voteHint: '每个用户每轮一票，可改投；每轮新检测重新计票。不通过票超限和其他未解除异常优先阻断恢复。',
        referenceAnswer: '参考答案（可选，仅向投票用户展示）',
        expectedAnswer: '预期答案（可选，不匹配时暂停）',
        answerMatch: '答案比较方式', exact: '完全一致', contains: '包含预期内容', numeric: '数值相等',
        rulesHint: '任一异常阈值命中即暂停；无数据或样本不足不判定通过或失败。至少为一个检测类型设置判断规则。百分比填写 0–100，延迟使用毫秒。',
        invalid: '请至少启用一条有效规则，并检查阈值、样本数、答案和投票门槛。',
