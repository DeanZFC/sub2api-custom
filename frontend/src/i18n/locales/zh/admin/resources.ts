export default {
    tests: {
      groupWorkflow: {
        ...
        description: '检测两个分组内的所有账号。糖果数值等于 21 时进入通过分组；数值不符或最终执行失败时进入不通过分组。管理员可直接判定鹈鹕，立即覆盖本轮分组。',
        publicVote: '允许用户对鹈鹕投票',
        publicVoteHint: '用户票数达到门槛时会切换分组；不通过门槛优先。管理员判定优先于用户投票，下轮重新计票。糖果仍只按数值 21 自动判断。',
        privateReviewHint: '默认关闭。当前仅管理员可判定鹈鹕；关闭用户投票不影响管理员按钮。',
      ...
      protection: {
        ...
        rulesHint: '无数据、样本不足或票数未达标时保持上次调度和分组。样本数按各指标分别判断；缓存率仅使用流式和 WebSocket 请求的有效缓存样本，同步请求不计入。百分比填写 0–100，延迟使用毫秒。解除暂停只解除本规则造成的暂停，其他规则仍可阻止恢复。',
