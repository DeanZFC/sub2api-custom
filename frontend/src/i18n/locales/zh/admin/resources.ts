export default {
    tests: {
      groupHint: '分组内每个可调度账号都会分别测试并保存结果。', cronHint: 'Cron 五项：分 时 日 月 周。例如 0 * * * * 为每小时运行；默认使用服务器时区，可在前面加 CRON_TZ=Asia/Shanghai 指定时区。',
      nextRun: '下次运行', runStarted: '测试已开始，完成后结果将自动更新。', resultsHint: '窗口打开时，每 5 秒自动更新测试结果。',
      title: '测试管理', description: '配置测试类型、分组、模型和定时计划', types: '测试类型', plans: '测试计划', name: '名称', key: '标识', type: '类型', target: '目标', group: '分组', account: '账号', selectType: '选择测试类型', selectGroup: '选择分组', selectAccount: '选择账号', model: '模型', schedule: '计划', cron: 'Cron 表达式', manual: '手动', run: '立即运行', results: '结果', kind: '输出类型', number: '数字', text: '文本', descriptionLabel: '说明', prompt: '提示词', maxResults: '保留结果数', htmlResult: 'HTML 测试结果'
    },
    scheduledTests: {
      title: '定时测试',
      addPlan: '添加计划',
