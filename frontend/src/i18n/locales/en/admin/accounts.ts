export default {
    accounts: {
      ...
      openai: {
        ...
        codexTicketAccountEnabled: 'Account-level 292 tickets',
        codexTicketAccountEnabledDesc: 'Applies only to this OpenAI OAuth / Setup Token account. When enabled, ticket harvesting is controlled by the account instead of the gateway-wide switch.',
        codexTicketFailClosed: 'Pause models without a ticket',
        codexTicketFailClosedDesc: 'Enabled by default. Pause the corresponding model when no valid 292 ticket is available; turn off to keep requests flowing while harvesting continues.',
        codexTicketHarvestProxies: 'Dedicated harvest proxies',
        codexTicketHarvestProxiesDesc: 'Select multiple proxies for ticket-harvest rotation. These do not change the account\'s business-request proxy. Tickets are not harvested without an available proxy.',
        codexTicketLegacyProxyFallback: 'This account still uses the legacy gateway harvest proxy. Select or clear the proxy list to switch to an account-level setting.',
