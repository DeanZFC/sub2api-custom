...
func bufferCodexRetrySSE(ctx context.Context, original io.ReadCloser, settings CodexPreOutputRetrySettings, limit int, timeout time.Duration) (*codexRetryBufferedBody, *openAICompatSSEFrame, error) {
...
func (s *OpenAIGatewayService) prepareCodexBufferedResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, firstOutputTimeout time.Duration) error {
	...
	// Keep the official first-output deadline when no progress has arrived.
	timeout := time.Duration(settings.MaxRetryWindowSeconds) * time.Second
	if firstOutputTimeout > 0 {
		timeout = min(timeout, max(time.Nanosecond, time.Until(startTime.Add(firstOutputTimeout))))
	}
	stopKeepalive := startOpenAISSEKeepalive(c, interval)
	body, failure, err := bufferCodexRetrySSE(ctx, resp.Body, settings, codexRetryHTTPBufferLimit, timeout)
	stopKeepalive()
	if err != nil {
		return err
	}
