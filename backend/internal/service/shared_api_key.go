...
func (s *SharedAPIKeyService) Create(ctx context.Context, userID int64, name, platform string, listings []int64) (*SharedAPIKey, error) {
	...
	v := &SharedAPIKey{UserID: userID, Name: strings.TrimSpace(name), Platform: strings.ToLower(strings.TrimSpace(platform)), Key: key, Status: StatusAPIKeyActive, ListingIDs: listings}
