...
func TestSharedCardsFetchPageAndRecentCallsInOneQuery(t *testing.T) {
	...
	now := time.Now().UTC()
	rows := sqlmock.NewRows([]string{"id", "account_id", "platform", "account_type", "display_name", "uploader_name", "status", "concurrency_limit", "concurrency_multiplier", "sell_rate", "total_call_count", "last_called_at", "calls"}).
		AddRow(7, 11, "openai", "apikey", "My account", "alice", "active", 3, 1, 2, 123, now, `[{"request_id":"r1","model":"","result_status":"success","duration_ms":10,"charged_amount":0.1,"created_at":"2026-09-09T00:00:00Z"}]`)
...
func TestSharedOwnerCardsIncludePausedAccountsAndEnforceOwner(t *testing.T) {
	...
	defer db.Close()
	repo := &sharedAccountPoolRepository{db: db}
	rows := sqlmock.NewRows([]string{"id", "account_id", "platform", "account_type", "display_name", "uploader_name", "status", "concurrency_limit", "concurrency_multiplier", "sell_rate", "total_call_count", "last_called_at", "calls"}).AddRow(7, 11, "openai", "oauth", "My account", "alice", "paused", 3, 1, 2, 0, nil, `[]`)
