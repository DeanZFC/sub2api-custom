...
func (r *sharedAccountPoolRepository) SetListingStatus(ctx context.Context, ownerID, listingID int64, status string) error {
	...
	defer tx.Rollback()
	var accountID int64
	err = tx.QueryRowContext(ctx, `UPDATE shared_account_listings SET status=$3,updated_at=NOW() WHERE id=$1 AND owner_user_id=$2 AND deleted_at IS NULL AND status IN ('active','paused') AND EXISTS (SELECT 1 FROM accounts a WHERE a.id=shared_account_listings.account_id AND a.account_scope='shared' AND a.status='active') RETURNING account_id`, listingID, ownerID, status).Scan(&accountID)
