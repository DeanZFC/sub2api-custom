...
func TestScheduledTestRetryResultHandler(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			svc.SetRetryFunc(func(_ context.Context, plan *service.ScheduledTestPlan, id int64) (*service.ScheduledTestResult, error) {
				if tc.runErr != nil {
					return nil, tc.runErr
				}
				return &service.ScheduledTestResult{ID: 10, PlanID: plan.ID, AccountID: &id, Status: "running"}, nil
			})
			router := gin.New()
